...
export const mockDNAResponse = {
  content: JSON.stringify({
    ...
    rooms: [
      { name: 'Living Room', dimensions: '5.5m × 4.0m', floor: 'ground', features: ['fireplace', 'bay window'] },
      { name: 'Kitchen', dimensions: '4.0m × 3.5m', floor: 'ground', features: ['island', 'pantry'] },
      { name: 'Master Bedroom', dimensions: '4.5m × 3.8m', floor: 'first', features: ['ensuite', 'built-in wardrobe'] }
    ],
    viewSpecificFeatures: {
      north: { mainEntrance: 'centered', windows: 4, features: ['porch', 'canopy'] },
      south: { patioDoors: 'large sliding', windows: 3, features: ['garden access'] },
...
export function mockTogetherFetch() {
  global.fetch = jest.fn((url, options) => {
    ...
    // Chat endpoint
    if (url.includes('/chat/completions') || url.includes('/api/together/chat')) {
    ...
    // Image endpoint
    if (url.includes('/images/generations') || url.includes('/api/together/image')) {
