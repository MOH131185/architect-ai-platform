...
const tests = [
  ...
  },
  {
    ...
    test: async () => {
      ...
      const { withConsistencyLock } = require('./src/services/a1SheetPromptGenerator');
      ...
      if (!locked.prompt.includes('CRITICAL CONSISTENCY LOCK')) {
        throw new Error('Consistency lock not applied');
      }
      
      if (!locked.prompt.includes(mockDesign.masterDNA.dimensions.length.toString())) {
  ...
  },
  {
    ...
    test: async () => {
      ...
      const { generateA1SheetImage } = require('./src/services/togetherAIService');
