...
console.log('✅ Prompt embedding dimension:', retrieved.prompts.embedding.length);
...
console.log('✅ Similarity (identical images):', (similarity_identical * 100).toFixed(2) + '%');
console.log('✅ Similarity (different images):', (similarity_different * 100).toFixed(2) + '%');
console.log('✅ Identical images have high similarity:', similarity_identical > 0.95);
console.log('✅ Different images have lower similarity:', similarity_different < similarity_identical, '\n');
...
const view1Status = view1Score >= 0.85 ? 'excellent' : view1Score >= 0.80 ? 'good' : 'acceptable';
...
const view2Status = view2Score >= 0.85 ? 'excellent' : view2Score >= 0.80 ? 'good' : 'acceptable';
...
const view3Status = view3Score >= 0.85 ? 'excellent' : view3Score >= 0.80 ? 'good' : view3Score >= 0.70 ? 'acceptable' : 'poor';
...
console.log('  Action:', view1Status === 'excellent' || view1Status === 'good' ? '✅ Accept' : '⚠️ Review');
console.log('');
...
console.log('  Action:', view2Status === 'excellent' || view2Status === 'good' ? '✅ Accept' : '⚠️ Review');
console.log('');
...
console.log('  Action:', view3Status === 'poor' ? '❌ Regenerate' : '⚠️ Review');
console.log('');
...
mockDNAPackage.consistency.history = [
  { timestamp: new Date().toISOString(), viewType: 'exterior_3d', score: view1Score, status: view1Status },
  { timestamp: new Date().toISOString(), viewType: 'elevation_north', score: view2Score, status: view2Status },
  { timestamp: new Date().toISOString(), viewType: 'section', score: view3Score, status: view3Status }
];
...
const avgScore = updated.consistency.history.reduce((sum, check) => sum + check.score, 0) / updated.consistency.history.length;
...
console.log('    Excellent (≥85%):', updated.consistency.history.filter(h => h.score >= 0.85).length);
console.log('    Good (80-84%):', updated.consistency.history.filter(h => h.score >= 0.80 && h.score < 0.85).length);
console.log('    Acceptable (70-79%):', updated.consistency.history.filter(h => h.score >= 0.70 && h.score < 0.80).length);
console.log('    Poor (<70%):', updated.consistency.history.filter(h => h.score < 0.70).length);
console.log('');
...
console.log('1. Review integration examples in src/examples/dnaWorkflowIntegrationExample.js');
