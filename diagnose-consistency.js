...
if (!fs.existsSync(envPath)) {
  ...
  if (
    !envContent.includes("TOGETHER_API_KEY=") ||
    envContent.includes("TOGETHER_API_KEY=your_")
  ) {
    issues.push(
      "❌ TOGETHER_API_KEY not set in .env - this is REQUIRED for DNA workflow",
    );
    issues.push("   Get key from: https://api.together.ai/settings/api-keys");
    issues.push(
      "   Add credits ($5-10): https://api.together.ai/settings/billing",
    );
  } else {
    ...
    if (keyMatch && keyMatch[1].startsWith("tgp_v1_")) {
      ...
      warnings.push(
        "⚠️  TOGETHER_API_KEY format looks incorrect (should start with tgp_v1_)",
      );
    }
  }
  ...
  if (
    !envContent.includes("REACT_APP_OPENAI_API_KEY=") ||
    envContent.includes("REACT_APP_OPENAI_API_KEY=your_")
  ) {
    warnings.push(
      "⚠️  REACT_APP_OPENAI_API_KEY not set - DNA generation will fail and fallback to legacy",
    );
  ...
  if (
    !envContent.includes("REACT_APP_GOOGLE_MAPS_API_KEY=") ||
    envContent.includes("REACT_APP_GOOGLE_MAPS_API_KEY=your_")
  ) {
    warnings.push(
      "⚠️  REACT_APP_GOOGLE_MAPS_API_KEY not set - location features limited",
    );
  } else {
...
const togetherServicePath = path.join(
  __dirname,
  "src",
  "services",
...
if (!fs.existsSync(togetherServicePath)) {
  ...
  if (delayMatch) {
    ...
    if (delay < 6000) {
      issues.push(
        `❌ Rate limit delay is too short: ${delay}ms - should be at least 6000ms (6 seconds)`,
      );
      issues.push(
        "   This will cause only 2/13 views to generate (429 errors)",
      );
      issues.push(
        `   Fix: Edit togetherAIService.js and set delayMs = 6000 or higher`,
      );
    } else if (delay >= 6000 && delay < 8000) {
      warnings.push(
        `⚠️  Rate limit delay is ${delay}ms - acceptable but 8000ms (8 seconds) is more reliable`,
      );
      passes.push("✅ Rate limit delay is >= 6 seconds (will work)");
    } else {
      passes.push(
        `✅ Rate limit delay is ${delay}ms (excellent - very reliable)`,
      );
    }
  } else {
...
// ========================================
// TEST 7: Check server.cjs has Together.ai endpoints
// ========================================
console.log("TEST 7: Checking server.cjs API endpoints...");
const serverPath = path.join(__dirname, "server.cjs");
if (!fs.existsSync(serverPath)) {
  issues.push("❌ server.cjs not found - API proxy required!");
  issues.push("   Run: npm run server (in separate terminal)");
} else {
  const serverContent = fs.readFileSync(serverPath, "utf-8");

  if (
    !serverContent.includes("/api/together/chat") ||
    !serverContent.includes("/api/together/image")
  ) {
    issues.push("❌ server.cjs missing Together.ai endpoints");
    issues.push("   Required: /api/together/chat and /api/together/image");
  } else {
    passes.push("✅ server.cjs has Together.ai endpoints configured");
  }
}
...
if (issues.length === 0 && warnings.length === 0) {
  ...
  console.log(
    "🔴 CRITICAL ISSUES FOUND - DNA workflow will NOT work until fixed\n",
  );
  ...
  if (issues.some((i) => i.includes("TOGETHER_API_KEY"))) {
    ...
    console.log(
      "4. Add $5-10 credits at https://api.together.ai/settings/billing",
    );
    console.log("5. Restart servers: npm run dev");
  } else if (issues.some((i) => i.includes("Rate limit delay"))) {
    console.log(
      "❌ Rate limit delay too short - only 2/13 views will generate",
    );
    console.log("");
    console.log("FIX:");
  ...
  if (warnings.some((w) => w.includes("REACT_APP_OPENAI_API_KEY"))) {
    ...
    console.log("");
    console.log(
      "Impact: System will fall back to legacy generation without DNA",
    );
  ...
  console.log("");
  console.log(
    "You can test now, but expect the warnings to affect functionality.",
  );
}
