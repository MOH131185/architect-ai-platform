...
try {
  const featureFlagsPath = path.join(__dirname, 'src', 'config', 'featureFlags.js');
  ...
  if (fs.existsSync(featureFlagsPath)) {
    ...
    if (content.includes('FEATURE_FLAGS') && content.includes('geometryFirst')) {
    ...
    if (content.includes('module.exports')) {
      success('CommonJS compatibility added');
      testsPassed++;
    }
  } else {
...
try {
  ...
  if (fs.existsSync(validatorsPath)) {
    ...
    if (validatorContent.includes('MIN_DOOR_WIDTH') && validatorContent.includes('0.8')) {
      success('MIN_DOOR_WIDTH = 0.8m (800mm) constant found');
      testsPassed++;
    }

    if (validatorContent.includes('MIN_WWR') && validatorContent.includes('0.25')) {
...
try {
  ...
  const buildGeomPath = path.join(__dirname, 'src', 'geometry', 'buildGeometry.ts');
  ...
  if (fs.existsSync(camerasPath)) {
    ...
    if (content.includes('createAxonometricCamera') &&
...
try {
  ...
  if (fs.existsSync(renderApiPath)) {
    ...
    if (content.includes('axon') && content.includes('persp') && content.includes('interior')) {
  ...
  if (fs.existsSync(hookPath)) {
    ...
    if (content.includes('axonUrl') && content.includes('perspUrl') && content.includes('interiorUrl')) {
      success('Separate state keys: axonUrl, perspUrl, interiorUrl (no shared var)');
      testsPassed++;
    }
  } else {
  ...
  const settingsPath = path.join(__dirname, 'src', 'components', 'GeometryFirstSettings.jsx');
...
try {
  ...
  if (fs.existsSync(planApiPath)) {
    ...
    if (content.includes('response_format') && content.includes('json_object')) {
      success('Response format: JSON (structured output)');
      testsPassed++;
    }
    if (content.includes('address') && content.includes('program') && content.includes('climate')) {
  ...
  const dnaGenPath = path.join(__dirname, 'src', 'services', 'togetherDNAGenerator.js');
...
try {
  ...
  if (fs.existsSync(sheetApiPath)) {
    ...
    if (content.includes('design_id') && content.includes('seed') && content.includes('sha256')) {
  ...
  const composerPath = path.join(__dirname, 'src', 'services', 'sheetComposer.js');
...
try {
  ...
  if (fs.existsSync(testsPath)) {
    ...
    if (content.includes('3 different URLs') || content.includes('byte sizes')) {
      success('Tests verify 3 different URLs & byte sizes');
      testsPassed++;
    }
    if (content.includes('/api/render') && content.includes('/api/plan') && content.includes('/api/sheet')) {
  ...
  if (fs.existsSync(readmePath)) {
    ...
    if (content.includes('Pipeline') || content.includes('graph TD') || content.includes('mermaid')) {
