      ...
      body: JSON.stringify({
        messages: [
          ...
          },
          {
            role: "user",
            content:
              'Generate a simple building DNA with dimensions: {"length": 10, "width": 8}',
          },
        ],
        temperature: 0.3,
        max_tokens: 500,
  ...
  try {
    ...
    const response = await fetch(`${API_BASE}/api/together/image`, {
      ...
      body: JSON.stringify({
        ...
        prompt:
          "Architectural floor plan, 2D CAD drawing, black lines on white background",
        width: 512,
        height: 512,
        seed: testSeed,
...
function printSummary() {
  ...
  console.log("\nTest Results:");
  console.log(
    `  DNA Generation: ${tests.dnaGeneration ? "✅ PASS" : "❌ FAIL"}`,
  );
  console.log(`  Reasoning: ${tests.reasoning ? "✅ PASS" : "❌ FAIL"}`);
  console.log(
    `  Image Generation: ${tests.imageGeneration ? "✅ PASS" : "❌ FAIL"}`,
  );
