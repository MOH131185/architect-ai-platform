...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(
      `[Sheet API] Exporting ${format.toUpperCase()} sheet for ${designId} (${sheetType})`,
    );
    ...
    if (format === "png" || format === "jpg" || format === "jpeg") {
      ...
      if (overlays.length > 0) {
        ...
        try {
          ...
          const overlayResponse = await fetch(
            `${process.env.API_BASE_URL || "http://localhost:3001"}/api/overlay`,
            {
              method: "POST",
          ...
          if (overlayResponse.ok) {
            ...
            console.warn(
              "[Sheet API] Overlay composition failed, using base image",
            );
          }
        } catch (overlayError) {
          console.warn(
            "[Sheet API] Overlay composition error:",
      ...
      const svg = createA1SheetSVG({
        ...
        views: {},
        siteMapImage:
          overlays.find((o) => o.type === "site-plan")?.dataUrl || null,
      });

      res.setHeader("Content-Type", "image/svg+xml");
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${designId}_${sheetType}_${versionId}.svg"`,
      );
...
function createA1SheetSVG(designData) {
  ...
  const materials = Array.isArray(masterDNA.materials)
    ? masterDNA.materials
    : [];
...
function renderEnvironmentalPanel(locationProfile, metrics) {
  ...
  const wwr = metrics?.fenestration?.wwr
    ? (metrics.fenestration.wwr * 100).toFixed(1) + "%"
    : "N/A";
