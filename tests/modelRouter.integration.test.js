...
describe('ModelRouter Integration Tests', () => {
  ...
  describe('Task Type Routing', () => {
    test('should route DNA_GENERATION to correct model', () => {
      process.env.AI_MODEL_DNA = 'meta-llama/Meta-Llama-3.1-405B-Instruct-Turbo';

      const config = modelRouter.getModelConfig('DNA_GENERATION');

      expect(config).toBeDefined();
      expect(config.primary).toBe('meta-llama/Meta-Llama-3.1-405B-Instruct-Turbo');
    });

    test('should route ARCHITECTURAL_REASONING to correct model', () => {
      process.env.AI_MODEL_REASONING = 'Qwen/Qwen2.5-72B-Instruct-Turbo';

      const config = modelRouter.getModelConfig('ARCHITECTURAL_REASONING');

      expect(config).toBeDefined();
      expect(config.primary).toBe('Qwen/Qwen2.5-72B-Instruct-Turbo');
    });
  ...
  describe('Fallback Cascade', () => {
    test('should have fallback model for DNA generation', () => {
      process.env.AI_MODEL_DNA = 'meta-llama/Meta-Llama-3.1-405B-Instruct-Turbo';
      process.env.AI_FALLBACK_DNA = 'Qwen/Qwen2.5-72B-Instruct-Turbo';

      const config = modelRouter.getModelConfig('DNA_GENERATION');

      expect(config.fallback).toBe('Qwen/Qwen2.5-72B-Instruct-Turbo');
    });
    ...
    test('should accept standard chat completion format', async () => {
      const options = {
        messages: [
          ...
          { role: 'user', content: 'Generate design DNA for a residential building.' }
        ],
        temperature: 0.7,
        max_tokens: 4000
      };
  ...
  describe('Image Generation Integration (Mock)', () => {
    test('should accept standard image generation format', async () => {
      const options = {
        prompt: 'Professional A1 architectural sheet with floor plans and elevations',
        width: 1792,
        height: 1269,
        steps: 48,
        guidance_scale: 7.8,
  ...
  describe('Cost Tracking', () => {
    test('should track token usage for LLM calls', () => {
      ...
      expect(mockUsage.total_tokens).toBe(mockUsage.prompt_tokens + mockUsage.completion_tokens);
    ...
    test('should calculate cost based on token usage', () => {
      ...
      const inputCostPerMillion = 3.00;   // $3/1M tokens
      const outputCostPerMillion = 3.00;  // $3/1M tokens
  ...
  describe('Error Handling', () => {
    ...
    test('should validate model availability before calling', () => {
      ...
      expect(() => {
        if (!unavailableModel.includes('llama') &&
  ...
  describe('Environment Configuration', () => {
    test('should switch models via environment variables', () => {
      ...
      // Test 2: Qwen 72B
      process.env.AI_MODEL_DNA = 'Qwen/Qwen2.5-72B-Instruct-Turbo';
