...
import { buildA1SheetPrompt } from "./src/services/a1/A1PromptService.js";
import a1SheetValidatorModule from "./src/services/a1/A1ValidationService.js";
const a1SheetValidator =
  a1SheetValidatorModule.default || a1SheetValidatorModule;

console.log(
  "🧪 Testing Clinic A1 Prompt Generation with Template Validation\n",
);
...
try {
  ...
  if (!templateValidation.valid) {
    ...
    console.error(
      "   Missing mandatory:",
      templateValidation.missingMandatory.join(", "),
    );
  ...
  console.log(
    `   Present sections: ${templateValidation.presentSections.length}`,
  );

  if (templateValidation.missingRecommended.length > 0) {
    console.log(
      `   ⚠️  Missing recommended: ${templateValidation.missingRecommended.join(", ")}`,
    );
  }
  ...
  if (
    !promptUpper.includes("NO SINGLE-FAMILY HOUSE") &&
    !promptUpper.includes("NO RESIDENTIAL HOUSE")
  ) {
  ...
  const sectionCount = ["SECTION A-A", "SECTION B-B"].filter((section) =>
    promptUpper.includes(section),
  ).length;
