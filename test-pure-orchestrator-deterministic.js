...
const { mockDNA, mockSiteSnapshot, mockDesignSpec, mockEnvironment } = require('./__mocks__/fixtures');
...
async function runTests() {
  ...
  assert(DRIFT_THRESHOLDS.DNA.OVERALL === 0.10, 'DNA drift threshold is 10%');
  assert(DRIFT_THRESHOLDS.IMAGE.SSIM_WHOLE === 0.92, 'Whole-sheet SSIM threshold is 92%');
  assert(DRIFT_THRESHOLDS.IMAGE.SSIM_PANEL === 0.95, 'Per-panel SSIM threshold is 95%');
  ...
  assert(modifyRequest.designId === 'design_123', 'ModifyRequest has correct design ID');
  ...
  console.log('\n📋 Test 10: Environment adapter');
  const environmentAdapter = require('./src/services/environmentAdapter').default;
  
  assert(environmentAdapter.currentEnv !== null, 'Environment adapter is initialized');
  assert(typeof environmentAdapter.currentEnv.api === 'object', 'API config is available');
