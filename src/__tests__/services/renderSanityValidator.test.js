...
describe("RenderSanityValidator Constants", () => {
  describe("Threshold Values", () => {
    test("MIN_OCCUPANCY_RATIO is 0.04 (4%) base threshold", () => {
      expect(MIN_OCCUPANCY_RATIO).toBe(0.04);
    });
...
describe("RenderSanityValidator Failure Detection Rules", () => {
  ...
  describe("Rule 4: Thin vertical strip", () => {
    test("5% threshold detects elevation compressed to narrow band", () => {
      ...
      const thinStripWidth = Math.floor(
        ANALYSIS_SIZE * THIN_STRIP_WIDTH_THRESHOLD,
      );
  ...
  describe("Rule 5: Thin horizontal strip", () => {
    test("5% threshold detects section compressed to thin band", () => {
      const thinStripHeight = Math.floor(
        ANALYSIS_SIZE * THIN_STRIP_HEIGHT_THRESHOLD,
      );
...
describe("RenderSanityValidator Integration Interface", () => {
  ...
  test("exports default object with all functions and constants", async () => {
    ...
    // Constants
    expect(defaultExport.MIN_OCCUPANCY_RATIO).toBe(0.04);
...
describe("RenderSanityValidator Documentation", () => {
  test("thresholds match documented base values in module JSDoc", () => {
    // These values are documented in the module header comments
    // Base occupancy floor is 4%; panel-specific and slot-aware thresholds
    // may tighten or relax around that base.
    expect(MIN_OCCUPANCY_RATIO).toBe(0.04);
  ...
  test("slot-aware thresholds relax fixed bbox floors for wide plan slots", () => {
    ...
    expect(thresholds.minOccupancyRatio).toBeLessThan(0.055);
    expect(thresholds.minOccupancyRatio).toBeGreaterThanOrEqual(0.02);
    expect(thresholds.minBboxHeightRatio).toBeGreaterThanOrEqual(
      THIN_STRIP_HEIGHT_THRESHOLD,
    );
  });
});
...
describe("RenderSanityValidator Image Processing (mocked)", () => {
  test("validateRenderSanity skips non-technical panels", async () => {
    const { validateRenderSanity } =
      await import("../../services/qa/RenderSanityValidator.js");
    ...
    expect(
      sanityCheck.warnings.some((w) =>
        w.includes("not subject to render sanity checks"),
      ),
    ).toBe(true);
  });
