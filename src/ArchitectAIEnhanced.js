...
// Generate DXF file content (AutoCAD-compatible text format)
const generateDXFContent = (projectDetails, bimModel = null) => {
  // If BIM model is available, use it to generate proper DXF
  if (bimModel) {
    try {
      console.log('Generating DXF from BIM model...');
      const dxfContent = bimService.exportToDWG(bimModel); // Returns DXF-like text format
      return dxfContent;
    } catch (error) {
      console.error('Failed to generate DXF from BIM model:', error);
      // Fall back to basic content
    }
  }

  // Basic DXF structure when no BIM model available
  ...
  return `999
ArchitectAI Platform - DXF Export
0
SECTION
2
HEADER
...
1.5
1
Total Area: ${projectDetails?.area || 'N/A'}m²
0
LINE
8
WALLS
      ...
      return (
        <div style={{
          ...
          {process.env.NODE_ENV === 'development' && (
            <details style={{ whiteSpace: 'pre-wrap', marginTop: '10px', fontSize: '12px', color: '#666' }}>
...
const ArchitectAIEnhanced = () => {
  ...
  // const [blendWeight, setBlendWeight] = useState(0.5); // DEPRECATED: Removed - no longer used
          ...
          <div className="grid md:grid-cols-3 gap-8 mb-16">
            {[
              ...
              },
              {
                ...
                description: "Export to all major CAD formats: DXF, RVT, IFC with full compatibility"
              }
            ].map((feature, idx) => (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <button
                    onClick={() => {
                      ...
                      const content = generateDXFContent(downloadDetails, generatedDesigns?.bimModel);
                      downloadFile('ArchitectAI_Design.dxf', content, 'application/dxf');
                      setDownloadCount(prev => prev + 1);
                      showToast('✓ DXF file downloaded successfully!');
                    }}
                  ...
                  >
                    ...
                    <span className="text-xs text-gray-600 mt-1">AutoCAD Drawing</span>
                  </button>
