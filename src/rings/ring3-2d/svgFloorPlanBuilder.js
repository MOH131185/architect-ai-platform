...
function ensureFootprint(dna) {
  ...
  const length =
    dna?.geometry?.footprint?.length || dna?.dimensions?.length || 20;
...
function buildRoomShapes(rooms, scale, padding) {
  ...
  return rooms
    ...
    .map((room) => {
      ...
      const centerX =
        room.vertices.reduce((acc, v) => acc + v.x, 0) / room.vertices.length;
      const centerY =
        room.vertices.reduce((acc, v) => acc + v.y, 0) / room.vertices.length;
      return `
