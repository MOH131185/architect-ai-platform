...
import {
  ...
  GENARCH_JOB_STATUSES,
  GENARCH_RESPONSE_VERSION_FIELD,
  GENARCH_VERSION_HEADER,
} from "../services/genarch/genarchContract.js";
...
function annotateContractVersion(operation, response, data) {
  const observedVersion =
    response?.headers?.get?.(GENARCH_VERSION_HEADER) ||
    data?.[GENARCH_RESPONSE_VERSION_FIELD] ||
    data?.job?.[GENARCH_RESPONSE_VERSION_FIELD] ||
    null;

  if (observedVersion && observedVersion !== GENARCH_CONTRACT_VERSION) {
    logger.warn("[GenarchService] Contract version mismatch", {
      operation,
      expected: GENARCH_CONTRACT_VERSION,
  ...
  if (
    ...
    typeof data === "object" &&
    !Array.isArray(data) &&
    !(GENARCH_RESPONSE_VERSION_FIELD in data)
  ) {
    return {
      ...data,
      [GENARCH_RESPONSE_VERSION_FIELD]: observedVersion,
    };
  }
  ...
  try {
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        seed: options.seed,
        skipPhase2: options.skipPhase2 ?? GENARCH_JOB_DEFAULTS.skipPhase2,
        skipPhase3: options.skipPhase3 ?? GENARCH_JOB_DEFAULTS.skipPhase3,
        skipPhase4: options.skipPhase4 ?? GENARCH_JOB_DEFAULTS.skipPhase4,
        strict: options.strict ?? GENARCH_JOB_DEFAULTS.strict,
        driftThreshold:
          options.driftThreshold ?? GENARCH_JOB_DEFAULTS.driftThreshold,
        waitForResult:
          options.waitForResult ?? GENARCH_JOB_DEFAULTS.waitForResult,
      }),
    });

    const data = annotateContractVersion(
      "createJob",
      response,
      await response.json(),
    );
...
export async function downloadArtifactBlob(jobId, relativePath) {
  ...
  try {
    ...
    const contractVersion =
      response.headers.get(GENARCH_VERSION_HEADER) || GENARCH_CONTRACT_VERSION;
