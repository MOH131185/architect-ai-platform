 ...
 *
 * @deprecated for A1 compose path. The runtime SSOT for A1 composition
 * layout is src/services/a1/composeCore.js (GRID_12COL / board-v2).
 *
 * This file is RETAINED for future multi-discipline support (STRUCTURE, MEP)
 * and is still imported by driftValidator.js, baselineArtifactStore.js, and
 * panelLayout.js. Do NOT use for A1 composition layout decisions.
 *
  ...
  for (let i = 0; i < panels.length; i++) {
    for (let j = i + 1; j < panels.length; j++) {
      ...
      if (p1.row === p2.row && p1.col === p2.col) {
        errors.push(
          `Panels ${p1.id} and ${p2.id} overlap at row ${p1.row}, col ${p1.col}`,
        );
      }
    }
  }
