...
export const useLocationData = () => {
  ...
  const getSeasonalClimateData = useCallback(async (lat, lon) => {
    try {
      ...
      const optimalFacadeOrientation = (() => {
        ...
        if (windSpeed > 5) {
          // Moderate to strong winds (> 18 km/h)
          if (windDeg >= 315 || windDeg < 45)
            return "Main rooms facing South (protected from North wind)";
          if (windDeg >= 45 && windDeg < 135)
            return "Main rooms facing West (protected from East wind)";
          if (windDeg >= 135 && windDeg < 225)
            return "Main rooms facing North (protected from South wind)";
          if (windDeg >= 225 && windDeg < 315)
            return "Main rooms facing East (protected from West wind)";
        }
        ...
        return {
          ...
          wind: {
            ...
            facadeRecommendation:
              "Main rooms facing East (protected from West wind)",
          },
          sunPath: {
  ...
  const analyzeLocation = useCallback(async () => {
    ...
    try {
      ...
      try {
        logger.info(
          "Generating Google Maps plan mode snapshot for A1 sheet",
          null,
          "🗺️",
        );
        ...
        if (snapshotResult && snapshotResult.dataUrl) {
          ...
          logger.info(
            "Google Maps plan snapshot generated successfully",
            null,
            "✅",
          );
        }
      ...
      logger.info(
        "Location analysis complete, advancing to step 2",
        null,
        "✅",
      );
      goToStep(2);
  ...
  const detectUserLocation = useCallback(async () => {
    ...
    try {
      ...
      if (error.message.includes("Geolocation is not supported")) {
        ...
        showToast(
          "Location services unavailable. Please enter address manually.",
        );
      } else {
        showToast(
   ...
   */
  const updateSitePolygon = useCallback(
    (polygon) => {
