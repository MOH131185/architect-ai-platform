...
export const useGeneration = () => {
   ...
   */
  const updateProgress = useCallback((phase, step, message) => {
    const totalSteps = 7;
    setGenerationProgress({
      phase,
      step,
    ...
    logger.info(`Progress: Step ${step}/${totalSteps} - ${message}`, null, '📊');
  }, [setGenerationProgress]);

  /**
   * Validate project parameters before generation
   */
  const validateBeforeGeneration = useCallback((projectContext) => {
    const errors = [];
    const warnings = [];

    logger.info('Validating project context', {
  ...
  const generateStyleSignature = useCallback(async () => {
    ...
    logger.info('Generating project style signature for DALL·E 3 consistency', null, '🎨');
  ...
  }, [projectStyleSignature, portfolioFiles, projectDetails, locationData, setProjectStyleSignature]);
  ...
  const analyzePortfolio = useCallback(async () => {
    ...
    logger.info('Analyzing uploaded portfolio', { count: portfolioFilesForAnalysis.length }, '🎨');
  ...
  const generateDesigns = useCallback(async () => {
    ...
    try {
      ...
      logger.info('Starting integrated AI design generation', {
        buildingProgram: projectContext.buildingProgram,
        area: projectContext.area,
        seed: projectSeed
      }, '🎨');
      ...
      logger.info('A1 Sheet available', { urlPreview: aiResult.a1Sheet.url?.substring(0, 80) + '...' }, '✅');
