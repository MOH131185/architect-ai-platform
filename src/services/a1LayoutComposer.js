 ...
 */
export async function composeA1Sheet({ panels = [], siteOverlay = null, layoutConfig = null, titleBlock = null }) {
  // Use dynamic import for sharp (server-side only)
  let sharp;
  try {
    sharp = (await import('sharp')).default;
  } catch (e) {
    throw new Error('sharp module not available - server-side composition requires sharp');
  }
  ...
  for (const [type, slot] of Object.entries(layout)) {
    ...
    if (type === 'title_block') {
      const titleBuffer = await buildTitleBlockBuffer(sharp, slotRect.width, slotRect.height - LABEL_HEIGHT - LABEL_PADDING, titleBlock);
      composites.push({
        input: titleBuffer,
        left: slotRect.x,
        top: slotRect.y
    ...
    if (panel?.buffer) {
      ...
      try {
        ...
        console.error(`[A1Composer] Failed to fetch panel ${type}:`, err.message);
        const placeholder = await buildPlaceholder(sharp, slotRect.width, slotRect.height - LABEL_HEIGHT - LABEL_PADDING, type);
  ...
  if (siteOverlay) {
    ...
    try {
      ...
      console.log(`[A1Composer] Added site overlay at (${slotRect.x}, ${slotRect.y})`);
    } catch (err) {
  ...
  console.log(`[A1Composer] Sheet composed: ${composites.length} elements, ${width}x${height}px`);
...
async function placePanelImage({ sharp, imageBuffer, slotRect, mode }) {
  ...
  try {
    ...
    if (Math.abs(imageAspect - slotAspect) / slotAspect > 0.2) {
      console.warn(`[A1Composer] Aspect ratio mismatch for panel: image ${imageAspect.toFixed(2)} vs slot ${slotAspect.toFixed(2)}`);
    }
  } catch (metaError) {
    console.warn('[A1Composer] Could not read image metadata:', metaError.message);
  }
