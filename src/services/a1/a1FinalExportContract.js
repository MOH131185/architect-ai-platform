...
function evaluateTechnicalPanelEvidence({
  panels,
  panelRegistry,
  ...
  const contract = validateTechnicalPanelContract({
    technicalPanels: panels,
    expectedGeometryHash,
  });
  ...
  for (const error of contract.errors || []) {
    const errorPanels = Array.isArray(error.details?.panels)
      ? error.details.panels
      : [error.panelType || "unknown"];
    for (const type of errorPanels) {
      if (error.code === "TECHNICAL_PANEL_IMAGE_MODEL_USED") {
  ...
  return {
    ...
    blank,
    missing,
    imageModelPanels: unique(imageModelPanels),
    blockedPanels,
    codes,
    contract,
  };
