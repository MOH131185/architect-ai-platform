...
import { embedFontInSVG, ensureFontsLoaded } from "../utils/svgFontEmbedder.js";

// Warm the font cache at module load so the first sheet render isn't blocked
// on font fetching. If it fails silently, embedFontInSVG still works per-call.
...
export async function generateUnifiedSheet(designResult, projectContext) {
  ...
  if (isFeatureEnabled("a1ProgrammaticComposer")) {
    ...
    try {
      ...
      const embedded = await embedFontInSVG(result.svg);
      return embedded;
    } catch (error) {
      logger.error(
...
<svg viewBox="0 0 ${SHEET_WIDTH} ${SHEET_HEIGHT}"
  ...
  <defs>
    <style>
      text { font-family: EmbeddedSans, 'Segoe UI', Arial, sans-serif; }
  ...
  return await embedFontInSVG(svg);
}
