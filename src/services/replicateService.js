...
class ReplicateService {
  ...
  buildElevationParameters(projectContext, direction = 'north') {
    ...
    // CRITICAL FIX: Detect building type to prevent apartment building style
    ...
    const isHouse = buildingTypeLC.includes('house') ||
                    ...
                    buildingTypeLC.includes('residential');

    // Building type emphasis to prevent wrong scale
    ...
    return {
      prompt: `MAXIMUM QUALITY professional CAD architectural elevation drawing, ${direction} ${elevationType} technical blueprint of ${buildingTypeEmphasis}, STRICTLY FLAT 2D FACADE VIEW ORTHOGRAPHIC PROJECTION, CRITICAL: THIS IS A ${unifiedDesc.floorCount}-FLOOR ${isHouse ? 'RESIDENTIAL HOUSE' : 'BUILDING'} NOT A ${unifiedDesc.floorCount > 2 ? 'MULTI-STORY APARTMENT BUILDING' : 'TALL BUILDING'}, CRITICAL CONSISTENCY REQUIREMENT: ${consistencyRule}, ULTRA-PRECISE CAD-QUALITY TECHNICAL DRAWING showing: EXACTLY ${unifiedDesc.floorCount} floor levels ONLY (${unifiedDesc.floorCount === 1 ? 'single story' : unifiedDesc.floorCount === 2 ? 'ground + first floor' : `ground + ${unifiedDesc.floorCount-1} upper floors`}) stacked vertically with clear floor division lines at ${floorHeight}m intervals, EXACT MATERIALS: ${specificMaterials} facade construction IDENTICAL to 3D views matching the brick house from photorealistic renderings, materials shown with professional architectural hatching patterns and textures indicating ${materialPrimary} (${materialColor}) with ${materialTexture}, ${windowSpec} shown as precise rectangles with glazing lines and mullions appropriate for ${isHouse ? 'residential house scale' : 'building scale'}, door openings with frame details and thresholds${isEntranceElevation ? ', main entrance door clearly articulated with threshold, frame, and canopy for residential house' : ''}, ground line marked (±0.00m), ${roofSpec} profile with edge detail and parapet/eaves matching residential house from 3D views, floor separation lines at ONLY ${unifiedDesc.floorCount} levels at ${floorHeight}m spacing (total height ${totalHeight}m NOT taller), COMPLETE PROFESSIONAL DIMENSIONING SYSTEM: vertical dimension chains showing ${unifiedDesc.floorCount} floor heights only (+0.00m${unifiedDesc.floorCount >= 2 ? `, +${floorHeight}m` : ''}${unifiedDesc.floorCount >= 3 ? `, +${floorHeight*2}m` : ''}), floor-to-floor heights (${floorHeight}m typical), total building height ${totalHeight}m dimension with extension lines, horizontal dimensions showing overall building width, facade bay modules and structural grid spacing appropriate for ${isHouse ? 'house' : 'building'}, window opening dimensions (height × width), door opening dimensions, dimension extension lines with clear arrows, dimension text in meters clearly labeled and legible, elevation level markers at EXACTLY ${unifiedDesc.floorCount} floor levels only, ground level reference datum (±0.00m), material legend showing ${materialPrimary} (${materialColor}), detail reference bubbles and section cut indicators, ORTHOGRAPHIC PROJECTION technical precision matching AutoCAD/Revit output quality, professional CAD-style architectural construction documentation with full dimensioning system and annotations, ultra-crisp clean black and white linework, maximum detail technical drawing, laser-sharp precise lines, professional construction documentation standards, MUST USE IDENTICAL MATERIALS AND COLORS as all other elevations (${materialPrimary} in ${materialColor}) and matching the brick house from 3D photorealistic views, unified consistent architectural design for ${unifiedDesc.floorCount}-story ${isHouse ? 'house' : 'building'} NOT apartment building`,
      buildingType: unifiedDesc.buildingType,
      architecturalStyle: unifiedDesc.architecturalStyle,
      materials: specificMaterials,
