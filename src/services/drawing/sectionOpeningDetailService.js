    ...
    .map((opening, index) => {
      ...
      return `
        <g id="phase13-section-cut-opening-${escapeXml(opening.id || index)}">
          ...
          <line x1="${opening.x}" y1="${opening.y}" x2="${opening.x + opening.width}" y2="${opening.y}" stroke="#111" stroke-width="${lineweights.primary || 1.2}"${contextual ? ' stroke-dasharray="5 4" stroke-opacity="0.78"' : ""} />
          <line x1="${opening.x}" y1="${opening.y + opening.height}" x2="${opening.x + opening.width}" y2="${opening.y + opening.height}" stroke="#111" stroke-width="${lineweights.primary || 1.2}"${contextual ? ' stroke-dasharray="5 4" stroke-opacity="0.78"' : ""} />
