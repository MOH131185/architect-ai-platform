...
export class FluxAIIntegrationService {
  ...
  async generateCompleteDesign(params) {
    ...
    console.log('📋 Project Context:', {
      ...
      generationMode: useUnifiedSheet ? 'UNIFIED A1 SHEET' : '13 SEPARATE IMAGES'
    });
    ...
    try {
      ...
      const finalResult = {
        ...
        workflow: 'dna-enhanced-together-ai',
  ...
  mapDNAResultsToLegacyFormat(dnaResult) {
    ...
    const floorPlans = {
      floorPlans: {
        ground: {
          images: dnaResult.floor_plan_ground?.success && dnaResult.floor_plan_ground?.url
        ...
        upper: {
          images: dnaResult.floor_plan_upper?.success && dnaResult.floor_plan_upper?.url
    ...
    const technicalDrawings = {
      technicalDrawings: {
        elevation_north: {
          images: dnaResult.elevation_north?.success && dnaResult.elevation_north?.url
            ? [dnaResult.elevation_north.url]
            : []
        },
        elevation_south: {
          images: dnaResult.elevation_south?.success && dnaResult.elevation_south?.url
            ? [dnaResult.elevation_south.url]
            : []
        },
        elevation_east: {
          images: dnaResult.elevation_east?.success && dnaResult.elevation_east?.url
            ? [dnaResult.elevation_east.url]
            : []
        },
        elevation_west: {
          images: dnaResult.elevation_west?.success && dnaResult.elevation_west?.url
            ? [dnaResult.elevation_west.url]
            : []
        },
        section_longitudinal: {
          images: dnaResult.section_longitudinal?.success && dnaResult.section_longitudinal?.url
            ? [dnaResult.section_longitudinal.url]
            : []
        },
        section_cross: {
          images: dnaResult.section_cross?.success && dnaResult.section_cross?.url
    ...
    const visualizations = {
      views: {
        exterior_front: {
          images: dnaResult.exterior_front_3d?.success ? [dnaResult.exterior_front_3d.url] : []
        },
        exterior_side: {
          images: dnaResult.exterior_side_3d?.success ? [dnaResult.exterior_side_3d.url] : []
        },
        interior: {
          images: dnaResult.interior_3d?.success ? [dnaResult.interior_3d.url] : []
        },
        axonometric: {
          images: dnaResult.axonometric_3d?.success ? [dnaResult.axonometric_3d.url] : []
        },
        perspective: {
          images: dnaResult.perspective_3d?.success ? [dnaResult.perspective_3d.url] : []
        }
      },
  ...
  async generateTechnicalDrawings() {
    ...
    for (const direction of elevations) {
      drawings[`elevation_${direction}`] = await togetherAIService.generateImage({
