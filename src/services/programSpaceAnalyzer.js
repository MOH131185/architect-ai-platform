...
class ProgramSpaceAnalyzer {
  ...
  getTemplateForType(category, subType) {
    ...
    const templateKey =
      categoryMappings[subType] || Object.values(categoryMappings)[0];
    return (
      this.buildingTypeTemplates[templateKey] ||
      this.buildingTypeTemplates["residential"]
    );
  }
  ...
  generateProgramFromSpecs(specs) {
    ...
    return this.generateSpaceProgram(
      specs.area,
      template,
      programData.specifications,
    );
  }
  ...
  validateProgramTable(programSpaces, constraints = {}) {
    ...
    programSpaces.forEach((space, index) => {
      ...
      if (space.label && spaceNames.has(space.label.toLowerCase())) {
        warnings.push(
          `Row ${index + 1}: Duplicate space name "${space.label}"`,
        );
      }
    ...
    if (constraints.targetArea && totalArea > 0) {
      ...
      if (percentDiff > 20) {
        warnings.push(
          `Total program area (${totalArea.toFixed(0)}m²) differs significantly from target (${constraints.targetArea}m²)`,
        );
      }
    }
  ...
  initializeCirculationRatios() {
    return {
      clinic: { min: 0.2, max: 0.3, typical: 0.25 },
      hospital: { min: 0.35, max: 0.45, typical: 0.4 },
      office: { min: 0.15, max: 0.25, typical: 0.2 },
      school: { min: 0.25, max: 0.35, typical: 0.3 },
      retail: { min: 0.1, max: 0.2, typical: 0.15 },
      residential: { min: 0.1, max: 0.2, typical: 0.15 },
      hotel: { min: 0.3, max: 0.4, typical: 0.35 },
    };
  }
  ...
  async analyzeProgram(projectData) {
    const { projectType, totalArea, siteArea, numberOfLevels, specifications } =
      projectData;

    // Get building template
    const template =
      this.buildingTypeTemplates[projectType] ||
      this.buildingTypeTemplates["residential"];
    ...
    const spaceProgram = this.generateSpaceProgram(
      totalArea,
      template,
      specifications,
    );
    ...
    const floorAnalysis = this.calculateOptimalFloors(
      totalArea,
      siteArea,
      template,
      projectData.zoning,
    ...
    const adjacencies = this.generateAdjacencyRequirements(
      projectType,
      spaceProgram,
    );

    // Check building code compliance
    const codeCompliance = this.checkCodeCompliance(
      spaceProgram,
      floorAnalysis,
      template,
    );
  ...
  validateArea(totalArea, template) {
    ...
    if (totalArea < template.minArea) {
      ...
      validation.issues.push(
        `Total area ${totalArea}m² is below minimum ${template.minArea}m² for ${template.name}`,
      );
    }

    if (totalArea > template.maxArea) {
      validation.warnings.push(
        `Total area ${totalArea}m² exceeds typical maximum ${template.maxArea}m² for ${template.name}`,
      );
    }
    ...
    if (totalArea < minRequiredArea * 1.3) {
      // Include circulation
      validation.warnings.push(
        `Tight fit: minimum required spaces need ${minRequiredArea}m² plus circulation`,
      );
    }
  ...
  generateSpaceProgram(totalArea, template, specifications) {
    ...
    const circulationRatio =
      this.circulationRatios[template.category]?.typical || 0.2;
    ...
    template.spaces.required.forEach((space) => {
      ...
      for (let i = 0; i < quantity; i++) {
        const spaceArea = this.calculateSpaceArea(
          space,
          remainingArea,
          totalArea,
        );
  ...
  calculateOptimalFloors(totalArea, siteArea, template, zoning) {
    ...
    if (zoning?.maxHeight) {
      ...
      analysis.reasoning.push(
        `Height limit of ${zoning.maxHeight} allows maximum ${maxFloorsByHeight} floors`,
      );
    }
    ...
    if (template.category === "healthcare" && template.preferredFloors === 1) {
      ...
      analysis.reasoning.push(
        "Single story preferred for accessibility and emergency evacuation",
      );
    ...
    if (analysis.recommended > 6) {
      analysis.reasoning.push(
        "Consider concrete frame or steel structure for heights above 6 floors",
      );
    }
  ...
  checkCodeCompliance(spaceProgram, floorAnalysis, template) {
    ...
    const totalOccupancy = Math.ceil(
      spaceProgram.grossArea / template.occupancyLoad,
    );
    const requiredExits = Math.ceil(
      totalOccupancy / codes.egress.maxOccupantsPerExit,
    );

    if (requiredExits > 1) {
      compliance.recommendations.push(
        `Provide minimum ${requiredExits} exits for ${totalOccupancy} occupants`,
      );
    }
    ...
    const floorLoad =
      codes.structural[`minFloorLoad${template.name.replace(" ", "")}`] ||
      codes.structural.minFloorLoadOffice;
    compliance.recommendations.push(
      `Design for minimum floor load of ${floorLoad} kN/m²`,
    );
    ...
    spaceProgram.spaces.forEach((space) => {
      if (space.minHeight < codes.fire.minCeilingHeight) {
        compliance.issues.push(
          `${space.name} height ${space.minHeight}m below minimum ${codes.fire.minCeilingHeight}m`,
        );
  ...
  calculateEfficiency(spaceProgram, template) {
    ...
    efficiency.netToGross =
      (spaceProgram.netArea / spaceProgram.grossArea) * 100;
    efficiency.circulationRatio =
      (spaceProgram.circulationArea / spaceProgram.grossArea) * 100;
    efficiency.spaceUtilization =
      (spaceProgram.totalProgramArea / spaceProgram.netArea) * 100;
  ...
  estimateCosts(spaceProgram, floorAnalysis, location) {
    ...
    return {
      ...
      breakdown: {
        ...
        envelope: Math.round(totalCost * 0.2),
        services: Math.round(totalCost * 0.25),
        finishes: Math.round(totalCost * 0.2),
  ...
  parseQuantity(quantity, totalArea) {
    ...
    if (typeof quantity === "string") {
      if (quantity.includes("-")) {
        ...
        return Math.max(
          min,
          Math.min(max, Math.round(min + (max - min) * ratio)),
        );
      }
  ...
  calculateSpaceArea(space, remainingArea, totalArea) {
    ...
    const targetArea =
      space.minArea +
      (space.maxArea - space.minArea) * Math.min(scaleFactor, 1);
  ...
  getAdjacencyType(priority) {
    switch (priority) {
      case 3:
        return "Critical - Direct connection required";
      case 2:
        return "Important - Close proximity preferred";
      case 1:
        return "Optional - Convenient if possible";
      default:
  ...
  getLocationCostMultiplier(location) {
    ...
    if (location?.includes("Birmingham")) return 1.1;
    if (location?.includes("Edinburgh")) return 1.2;
    return 1.0; // Default UK average
  }
}

export default new ProgramSpaceAnalyzer();
