...
class ModelSelector {
  ...
  selectModel(taskType, context = {}) {
    ...
    if (!modelConfig) {
      console.warn(
        `Unknown task type: ${taskType}, using DNA_GENERATION default`,
      );
    ...
    if (budget === "low" && modelConfig.primary.costPerCall > 0.02) {
      if (
        ...
        modelConfig.fallback.costPerCall < modelConfig.primary.costPerCall
      ) {
    ...
    if (timeConstraint === "urgent") {
      ...
      if (primaryLatency > 30 && modelConfig.fallback) {
        const fallbackLatency = this.parseLatency(
          modelConfig.fallback.avgLatency,
        );
  ...
  calculateCost(taskType, iterations = 1, context = {}) {
    ...
    if (
      ...
      modelConfig.model.includes("Qwen")
    ) {
    ...
    return {
      ...
      breakdown: {
        ...
        tokens: context.estimatedTokens
          ? this.calculateTokenCost(modelConfig.model, context.estimatedTokens)
          : 0,
      },
    };
  }
    ...
    const workflows = {
      full_project: {
        steps: [
          ...
          {
            task: "A1_SHEET_GENERATION",
            model: "FLUX.1-kontext-max",
            cost: 0.025,
          },
          {
            task: "MODIFICATION_REASONING",
            model: "meta-llama/Llama-3.3-70B-Instruct-Turbo",
            cost: 0.025,
          },
          {
            task: "MODIFICATION_IMAGE",
            model: "FLUX.1-kontext-max",
            cost: 0.025,
          },
        ],
  ...
  addDynamicParameters(model, context) {
    ...
    if (context.priority === "speed" && dynamic.params.num_inference_steps) {
      dynamic.params.num_inference_steps = Math.max(
        4,
        Math.floor(dynamic.params.num_inference_steps * 0.5),
      );
    } else if (
      context.priority === "quality" &&
      dynamic.params.num_inference_steps
    ) {
      dynamic.params.num_inference_steps = Math.min(
        50,
        Math.floor(dynamic.params.num_inference_steps * 1.2),
      );
    }
