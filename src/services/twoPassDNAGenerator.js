...
class TwoPassDNAGenerator {
  ...
  async generateMasterDesignDNA(projectContext, portfolioAnalysis = null, locationData = null) {
    ...
    const effectiveLocation = locationData || projectContext.location || projectContext.locationData;
    const siteMetrics = projectContext.siteMetrics || projectContext.siteAnalysis;
    const programSpec = {
      floors: projectContext.floorCount || projectContext.programSpaces?._calculatedFloorCount || 2,
    ...
    const rawDNA = await this.passA_generateStructuredDNA(requestPayload, projectContext);

    if (!rawDNA) {
      throw new Error('Pass A failed: Unable to generate DNA. Please check AI service availability.');
    }

    // PASS B: Reviewer - Validate and repair
    logger.info('🔹 PASS B: Validating and repairing DNA (Reviewer)...');
    const validatedDNA = await this.passB_validateAndRepair(rawDNA, requestPayload, projectContext);
...
Generate the corrected DNA now (JSON only):`;
    ...
    try {
      ...
      if (revalidation.valid) {
        ...
        logger.warn('⚠️  AI repair incomplete, applying deterministic repair...');
        // Fall through to deterministic repair
      }

    } catch (error) {
      logger.warn('⚠️  AI repair failed, applying deterministic repair...', error.message);
      // Fall through to deterministic repair
    }
