...
import { detectDNADrift, detectImageDrift, suggestDriftCorrections, DRIFT_THRESHOLDS } from './driftValidator.js';
...
export async function modifySheet({
  ...
  logger.info('Starting pure modification workflow', { designId: designRef.id }, '🔧');
  
  // Normalize modify request
  const normalizedRequest = createModifyRequest(modifyRequest);
  const geometryVolumeEnabled = featureFlags.geometryVolumeFirst ?? isFeatureEnabled('geometryVolumeFirst');
  ...
  try {
    ...
    if (!baseline) {
      ...
      logger.warn('Baseline artifacts not found in store, attempting to load from design history', { designId: designRef.id });
      ...
      if (!design.resultUrl && !design.a1Sheet?.url) {
        logger.error('Design missing baseline image URL', { designId: designRef.id });
        throw new Error(
      ...
      const reconstructedBaseline = {
        ...
        metadata: {
          ...
          a1LayoutKey: design.a1Sheet?.metadata?.a1LayoutKey || 'uk-riba-standard'
        },
    ...
    if (!geometryDNA || !geometryRenders) {
      if (!designFromHistory) {
        try {
          designFromHistory = await designHistoryRepository.getDesignById(designRef.id);
        } catch (historyError) {
          logger.warn('Failed to load design for geometry baseline', { error: historyError.message });
        }
      }
    ...
    if (geometryVolumeEnabled) {
      try {
        ...
        logger.warn('Classification failed, falling back to heuristic', { error: classErr.message });
        modificationClass = modificationClassifier.heuristicClassification(deltaText || normalizedRequest.customPrompt || '');
      }
    }
    ...
    if (needsGeometryRegen || (geometryVolumeEnabled && geometryDNA && !geometryRenders)) {
      geometryDNA =
        geometryDNA ||
        generateGeometryDNA({
      ...
      geometryBaselineSource = needsGeometryRegen ? 'regenerated' : geometryBaselineSource || 'synthesized';
    ...
    if (geometryVolumeEnabled && geometryDNA && geometryRenders && (!resolvedGeometry.geometryDNA || needsGeometryRegen)) {
      try {
        ...
        logger.info('Geometry baseline persisted for modify workflow', { source: geometryBaselineSource });
      } catch (persistErr) {
        logger.warn('Failed to persist geometry baseline (non-fatal)', { error: persistErr.message });
      }
    }
    ...
    const geometryDirective = geometryDNA
      ...
      : '';
    const deltaPrompt = geometryDirective ? `${deltaText}; ${geometryDirective}` : deltaText;
    const dnaForPrompt = {
      ...baseline.baselineDNA,
    ...
    const baseStrength = normalizedRequest.imageStrength || determineOptimalStrength(normalizedRequest);
    const imageStrength = tuneStrengthForClassification(
      baseStrength,
      modificationClass?.classification,
      geometryVolumeEnabled
    );
    const modificationType = pickModificationType(modificationClass?.classification);
    
    logger.info('Using img2img settings', {
      seed: baseline.metadata.seed,
    ...
    if (driftAnalysis.hasDrift && driftAnalysis.driftScore > DRIFT_THRESHOLDS.DNA.OVERALL) {
      ...
      if (retryDriftAnalysis.driftScore < driftAnalysis.driftScore) {
        ...
        logger.success('Retry successful', { driftScore: retryDriftAnalysis.driftScore });
      } else {
        // Fail gracefully
...
function determineOptimalStrength(request) {
  ...
  if (quickToggles.addDetails && !quickToggles.addSections && !quickToggles.add3DView) {
    return 0.10;
  }
  
  // Adding views: moderate changes (85% preserve)
  if (quickToggles.addSections || quickToggles.add3DView || quickToggles.addFloorPlans) {
    return 0.15;
  }
...
async function loadImageAsDataURL(imageUrl) {
  ...
  try {
    ...
    const fetchUrl = imageUrl.startsWith('http://localhost') || imageUrl.startsWith(window.location.origin)
      ? imageUrl
...
async function detectDrift({
  ...
  if (baselineUrl && candidateUrl) {
    imageDrift = await detectImageDrift(baselineUrl, candidateUrl, { panelCoordinates });
  }
...
function resolveGeometryFromBaseline(baseline) {
  if (!baseline) return { geometryDNA: null, geometryRenders: null, geometryScene: null, source: null };
  ...
  const geometryRenders =
    ...
    null;
  const geometryScene = geometryBaseline?.scene || geometryBaseline?.geometryScene || null;
...
function resolveGeometryFromDesign(design) {
  if (!design) return { geometryDNA: null, geometryRenders: null, geometryScene: null };
  return {
    geometryDNA:
      design.geometryDNA ||
...
function summarizeGeometryDNA(geometryDNA) {
  ...
  const floors = geometryDNA.floorHeights?.length || geometryDNA.volumes?.[0]?.levels || null;
  const wings = Array.isArray(geometryDNA.wings) ? geometryDNA.wings.join('/') : 'bar';
...
function tuneStrengthForClassification(baseStrength, classification, enabled) {
  ...
  if (classification === 'minor_elevation') return Math.min(Math.max(baseStrength, 0.14), 0.18);
  if (classification === 'volume_change' || classification === 'new_project') return Math.max(baseStrength, 0.24);
  return baseStrength;
}
...
function pickModificationType(classification) {
  ...
  if (classification === 'volume_change' || classification === 'new_project') return 'significant';
  return 'moderate';
}
