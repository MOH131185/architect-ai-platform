...
Classify now (JSON only):`;
    ...
    try {
      ...
      logger.info(
        "   Requires geometry regen: " +
          classification.requires_geometry_regeneration,
      );
      ...
      logger.warn(
        "⚠️  Classification failed, using heuristic fallback:",
        error.message,
      );
  ...
  heuristicClassification(modificationRequest) {
    ...
    const hasAppearanceKeyword = appearanceKeywords.some((kw) =>
      request.includes(kw),
    );
    ...
    const hasElevationKeyword = elevationKeywords.some((kw) =>
      request.includes(kw),
    );
