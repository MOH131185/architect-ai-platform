...
class DesignGenerationHistory {
  ...
  recordModificationResult(sessionId, modificationId, result) {
    ...
    if (result.success) {
      if (result.type === 'a1-sheet') {
        ...
        if (category) {
          session.currentState.individualViews[category.category][category.key] = result.data;
        }
      }
    }
  ...
  getMissingViews(sessionId) {
    ...
    const standardViews = [
      ...
      { category: 'technicalDrawings', key: 'longitudinal', label: 'Longitudinal Section' },
      { category: 'technicalDrawings', key: 'transverse', label: 'Transverse Section' },
  ...
  categorizeView(viewType) {
    const mapping = {
      ...
      'longitudinal-section': { category: 'technicalDrawings', key: 'longitudinal' },
      'transverse-section': { category: 'technicalDrawings', key: 'transverse' },
