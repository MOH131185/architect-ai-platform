...
import { createSheetResult, normalizeMultiPanelResult } from '../types/schemas.js';
...
import { getA1Preset, shouldRetryForDrift, calculateRetryStrength, getModifyStrength } from '../config/fluxPresets.js';
...
export async function runA1SheetWorkflow(params) {
  ...
  return {
    ...
    a1Sheet: {
      ...
      coordinates: multiPanelResult.panelCoordinates || multiPanelResult.coordinates
    },
    coordinates: multiPanelResult.panelCoordinates || multiPanelResult.coordinates,
    panels: multiPanelResult.panelMap || multiPanelResult.panels,
    panelMap: multiPanelResult.panelMap || multiPanelResult.panels,
...
export async function runModifyWorkflow(params) {
  ...
  logger.info('Starting modify workflow', {
  ...
  try {
    ...
    if (!baseline) {
      throw new Error('Baseline artifacts not found - cannot modify without baseline');
    }
    ...
    if (geometryVolumeEnabled) {
      try {
        ...
        modificationClass = modificationClassifier.heuristicClassification(deltaText || modifyRequest.customPrompt || '');
      }
    }
    ...
    const geometryDirective = geometryDNA
      ...
      : '';
    const deltaPrompt = geometryDirective ? `${deltaText}\n${geometryDirective}` : deltaText;
    const dnaForPrompt = {
      ...baseline.baselineDNA,
    ...
    const promptResult = buildSheetPrompt({
      dna: dnaForPrompt,
      siteSnapshot: params.siteSnapshot,
      sheetConfig: params.sheetConfig || { size: 'A1', orientation: 'landscape' },
    ...
    logger.info('STEP 4: Generating modified image with A1_ARCH_FINAL preset', null, '??');
    const client = createTogetherAIClient(env);
    const preset = getA1Preset('modify');
    
    const defaultModificationType = modifyRequest.modificationType || 
                            (modifyRequest.quickToggles?.addDetails ? 'minor' : 'moderate');
    const modificationType = pickModificationType(modificationClass?.classification, defaultModificationType);
    const baseStrength = modifyRequest.imageStrength || getModifyStrength(modificationType);
    const tunedStrength = tuneStrengthForClassification(baseStrength, modificationClass?.classification, geometryVolumeEnabled);
    
    logger.info('Using modify preset', {
      steps: preset.steps,
      cfg: preset.cfg,
      modificationType,
    ...
    if (driftDecision.shouldRetry) {
      ...
      const originalStrength = imageResult.metadata.imageStrength || tunedStrength;
      const retryStrength = calculateRetryStrength(originalStrength, 1);
      
      logger.info('Retry with reduced strength', {
      ...
      if (retryDecision.shouldFail) {
        throw new Error(`Drift too high after retry: ${retryDriftAnalysis.driftScore.toFixed(3)} (threshold: ${preset.driftThresholds.fail}). Simplify modification request.`);
      }
    }
