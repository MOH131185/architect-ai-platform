/**
 * Design History Service — DEPRECATED COMPATIBILITY FACADE
 *
 * @deprecated Active flows should use designHistoryRepository.js (canonical store).
 * This service is retained only for backward compatibility with legacy callers
 * (e.g. example scripts, import/export utilities). No active production flow
 * should write through this service. If you need to persist a design, use
 * designHistoryRepository.saveDesign() instead.
 *
 * Backend: localStorage via storageManager (key: "design_history" — same as repository)
 * Risk: writes here can overwrite repository data with an older schema shape.
 */
...
import {
  saveSiteSnapshot,
  deleteSiteSnapshot,
} from "../utils/siteSnapshotStore.js";
  ...
  async getAllHistory() {
    try {
      ...
      if (stored && typeof stored === "object") {
        logger.warn(
          "⚠️ Detected corrupted design history (object instead of array), migrating...",
        );
        ...
        if (keys.length > 0) {
          ...
          await storageManager.setItem(
            this.storageKey,
            trimHistoryForStorage(repaired),
          );
...
`;
  ...
  async createDesign(params) {
    try {
      ...
      if (siteSnapshot?.dataUrl) {
        siteSnapshotKey = await saveSiteSnapshot(
          resolvedDesignId,
      ...
      if (sheetUrl !== strippedSheetUrl) {
        logger.info(
          `   Stripped A1 sheet URL: ${(sheetUrl?.length / 1024).toFixed(2)}KB → metadata only`,
        );
      }
      if (siteSnapshot?.dataUrl && !siteSnapshotKey) {
        logger.warn(
          "⚠️ Site snapshot could not be persisted; overlay consistency may drift.",
        );
      }
      ...
      const resolvedSheetMetadata = sanitizeSheetMetadataHelper(
        a1SheetMetadata || a1Sheet?.metadata || {},
      );
      const resolvedPanelLayout = resolvePanelLayout(
        a1SheetPanels ||
          a1Sheet?.panelLayout ||
      ...
      const resolvedPanelMap = resolvePanelMap(
        panelMap ||
          params.panels ||
          a1Sheet?.panelMap ||
          a1Sheet?.panels ||
      ...
      const sanitizedGeometryRenders = sanitizeGeometryRenders(
        geometryRenders || a1Sheet?.geometryRenders,
      );
      ...
      const design = {
        ...
        panelCoordinates:
          a1Sheet?.panelCoordinates ||
          a1Sheet?.coordinates ||
          resolvedSheetMetadata.coordinates ||
          null,
        ...
        siteSnapshot: siteSnapshotKey
          ? {
              ...
              metadata: {
                ...
                polygonStyle: siteSnapshot?.polygonStyle || {
                  strokeColor: "red",
                  strokeWeight: 2,
                  fillColor: "red",
                  fillOpacity: 0.2,
                ...
                capturedAt:
                  siteSnapshot?.capturedAt || new Date().toISOString(),
              },
            }
          : null,
      };
      ...
      if (history.length > MAX_DESIGNS) {
        logger.warn(
          `⚠️ History exceeds ${MAX_DESIGNS} designs, removing oldest...`,
        );
      ...
      stripDataUrlsDeep(design, {
        logSummary: true,
        label: "design history entry",
      });
      ...
      if (!saved) {
        ...
        logger.error(
          "   - Check browser console above for detailed storage error",
        );
        throw new Error(
          `Failed to save design to storage. Storage usage: ${storageUsage}%. Check console for detailed error.`,
        );
      }
  ...
  async getOrCreateDesign(designId, baseData = {}) {
    try {
      ...
      if (!design) {
        logger.info(
          `⚠️ Design ${designId} not found in history, creating entry...`,
        );
  ...
  async addVersion(designId, versionData) {
    try {
      ...
      if (strippedVersionData.resultUrl) {
        strippedVersionData.resultUrl = this.stripDataUrl(
          strippedVersionData.resultUrl,
        );
      }
      if (strippedVersionData.composedSheetUrl) {
        strippedVersionData.composedSheetUrl = this.stripDataUrl(
          strippedVersionData.composedSheetUrl,
        );
      }
      if (strippedVersionData.geometryRenders) {
        strippedVersionData.geometryRenders = sanitizeGeometryRenders(
          strippedVersionData.geometryRenders,
        );
      }
      if (strippedVersionData.panelMap) {
        strippedVersionData.panelMap = sanitizePanelMap(
          strippedVersionData.panelMap,
        );
      }
      if (strippedVersionData.panels) {
        strippedVersionData.panels = sanitizePanelMap(
          strippedVersionData.panels,
        );
      }
      ...
      if (index >= 0) {
        ...
        const saved = await storageManager.setItem(
          this.storageKey,
          trimmedHistory,
        );
        if (!saved) {
          ...
          throw new Error(
            `Failed to save version to storage. Storage usage: ${storageUsage}%`,
          );
        }
  ...
  async importHistory(file) {
    try {
      ...
      stripDataUrlsDeep(merged, {
        logSummary: true,
        label: "design_history import",
      });
      await storageManager.setItem(
        this.storageKey,
        trimHistoryForStorage(merged),
      );
