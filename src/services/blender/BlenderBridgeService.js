 ...
 *
 * @module services/blender/BlenderBridgeService
 */
 ...
 */
export class BlenderBridgeService {
  constructor(config = {}) {
  ...
  determineMode() {
    if (isServerless && !this.config.workerUrl) {
      logger.warn('[BlenderBridge] Serverless environment without worker URL - DISABLED');
      return 'disabled';
    }
    if (this.config.workerUrl) {
      logger.info(`[BlenderBridge] Worker mode: ${this.config.workerUrl}`);
      return 'worker';
    }
    logger.info('[BlenderBridge] Local mode: using local Blender installation');
    return 'local';
  }
  ...
  async checkAvailability() {
    ...
    if (this.mode === 'worker') {
      // For worker mode, check worker health
    ...
    try {
      ...
      logger.info(
        `[BlenderBridge] Local availability: ${this.isAvailable ? 'AVAILABLE' : 'NOT AVAILABLE'}`
      );
      return this.isAvailable;
    } catch (err) {
      this.isAvailable = false;
      logger.warn('[BlenderBridge] Local Blender not available:', err.message);
      return false;
    }
  }
  ...
  async checkWorkerHealth() {
    try {
      ...
      logger.warn('[BlenderBridge] Worker health check failed:', err.message);
      return false;
    }
  }
  ...
  async downloadGLB(modelUrl, outputPath) {
    logger.info(`[BlenderBridge] Downloading GLB from ${modelUrl}`);
    ...
    logger.info(
      `[BlenderBridge] GLB saved: ${outputPath} (${(buffer.length / 1024).toFixed(1)} KB)`
    );
  ...
  async renderViewsLocal(designStatePath, outputDir, runId) {
    logger.info('[BlenderBridge] Starting LOCAL Blender render...');
    ...
    logger.debug('[BlenderBridge] Running:', this.config.blenderPath, args.join(' '));

    const result = await this.runBlenderCommand(args);

    if (!result.success) {
      logger.error('[BlenderBridge] Render failed:', result.stderr);
      throw new Error(
    ...
    logger.info(`[BlenderBridge] Local render complete: ${manifest.views?.length || 0} views`);
  ...
  async renderViewsWorker(designState, runId) {
    logger.info(`[BlenderBridge] Starting WORKER render via ${this.config.workerUrl}...`);
  ...
  async pollWorkerTask(taskId) {
    ...
    for (let i = 0; i < maxPolls; i++) {
      ...
      logger.debug(`[BlenderBridge] Worker task ${taskId}: ${status.status}`);
    }
  ...
  async generateTechnicalDrawings(meshyResult, dna, options = {}) {
    ...
    if (!isFeatureEnabled('blenderTechnical')) {
      logger.info('[BlenderBridge] Blender technical drawings disabled (feature flag)');
      return { success: false, reason: 'feature_disabled', panels: {}, manifest: null };
    }

    // Check mode
    if (this.mode === 'disabled') {
      logger.warn('[BlenderBridge] Blender disabled in serverless environment without worker');
    ...
    if (!available) {
      logger.warn('[BlenderBridge] Blender not available');
    ...
    try {
      ...
      if (this.mode === 'local') {
        ...
        logger.debug('[BlenderBridge] DesignState saved:', designStatePath);
        ...
        for (const view of blenderManifest.views || []) {
          try {
            ...
            logger.warn(`[BlenderBridge] Failed to read ${view.id}:`, readErr.message);
          }
        }
      } else {
        // Worker mode
        ...
        if (workerResult.renders) {
          ...
          for (const [viewId, renderInfo] of Object.entries(workerResult.renders)) {
            try {
              ...
              logger.warn(`[BlenderBridge] Failed to download ${viewId}:`, err.message);
            }
          }
        }
      }
      ...
      logger.info(`[BlenderBridge] Generated ${Object.keys(mappedPanels).length} technical panels`);
      logger.info(`[BlenderBridge] Run manifest: ${manifestPath}`);
      ...
      logger.error('[BlenderBridge] Pipeline failed:', error.message);
  ...
  async cleanup(tempDir) {
    try {
      ...
      logger.debug(`[BlenderBridge] Cleaned up: ${tempDir}`);
    } catch (err) {
      logger.warn(`[BlenderBridge] Cleanup failed: ${err.message}`);
    }
  }
...
export const blenderBridgeService = new BlenderBridgeService();

// Export class for custom instances
export default BlenderBridgeService;
