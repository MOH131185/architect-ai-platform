...
const { derivePanelSeeds, derivePanelSeedsFromDNA, hashDNA } = require('./src/services/seedDerivation.js');
...
const panelKeys = ['hero_3d', 'interior_3d', 'floor_plan_ground', 'elevation_north'];
...
const formulaMatch = Object.keys(expectedSeeds).every(key => seeds1[key] === expectedSeeds[key]);
if (formulaMatch) {
...
const panelKeysForDNA = ['hero_3d', 'floor_plan_ground', 'elevation_north', 'section_AA'];
...
if (test5Pass) {
  ...
  console.log('❌ Test 5 FAILED: derivePanelSeedsFromDNA produces different results');
...
const wrappedSeeds = derivePanelSeeds(largeSeed, ['panel1', 'panel2', 'panel3']);
const allWrappedInRange = Object.values(wrappedSeeds).every(seed => seed >= 0 && seed <= 999999);
