...
try {
  ...
  const windImpact = windSpeed > 5 ? 'Moderate-High' : windSpeed > 3 ? 'Moderate' : 'Low';
...
try {
  ...
  console.log(`   Exceeds buildable area: ${exceedsFootprint ? 'YES (ERROR)' : 'NO (OK)'}`);
  console.log(`   Height: ${mockDNA.dimensions.height}m vs max ${mockSiteData.maxHeight}m`);
  console.log(`   Exceeds height limit: ${exceedsHeight ? 'YES (ERROR)' : 'NO (OK)'}`);

  if (exceedsFootprint || exceedsHeight || exceedsFloors) {
    console.log('   ✅ PASS: Site constraints detected violations (as expected)');
    console.log('   ✅ Auto-correction should reduce dimensions to fit');

    // Simulate auto-correction
    const scaleFactor = Math.sqrt(mockSiteData.buildableArea / footprintArea) * 0.95;
    ...
    console.log(`   ✅ Corrected footprint: ${correctedLength}m × ${correctedWidth}m = ${correctedFootprint}m²`);
    console.log(`   ✅ Now fits within ${mockSiteData.buildableArea}m²: ${correctedFootprint <= mockSiteData.buildableArea ? 'YES' : 'NO'}`);
  } else {
  ...
  const impossibleFootprint = impossibleDNA.dimensions.length * impossibleDNA.dimensions.width; // 10,000 m²
  const impossibleToFit = impossibleFootprint > mockSiteData.buildableArea * 10; // Way over

  if (impossibleToFit) {
    console.log('   ✅ PASS: Impossible constraint detected (should throw error)');
    console.log('   ✅ Expected error: "SITE CONSTRAINT VIOLATION: Cannot fit..."');
  }
...
`;
  ...
  console.log(`   ✅ Ground floor principles: ${hasGroundPrinciples ? 'Present' : 'Missing'}`);
  console.log(`   ✅ First floor principles: ${hasFirstPrinciples ? 'Present' : 'Missing'}`);
  console.log(`   ✅ Healthcare-specific rules: ${hasHealthcareRules ? 'Present' : 'Missing'}`);
  console.log(`   ✅ Accessibility logic: ${hasAccessibilityLogic ? 'Present' : 'Missing'}`);

  if (hasGroundPrinciples && hasFirstPrinciples && hasHealthcareRules && hasAccessibilityLogic) {
    console.log('   ✅ PASS: AI prompt includes intelligent level assignment logic');
  } else {
  ...
  const groundFloorSpaces = mockClinicSpaces.filter(s => s.level === 'Ground');
  const firstFloorSpaces = mockClinicSpaces.filter(s => s.level === 'First');

  console.log(`   ✅ Ground floor spaces: ${groundFloorSpaces.length} (public/treatment)`);
  console.log(`   ✅ First floor spaces: ${firstFloorSpaces.length} (private/admin)`);

  const groundHasPublicSpaces = groundFloorSpaces.some(s =>
    s.name.toLowerCase().includes('reception') ||
    s.name.toLowerCase().includes('consultation') ||
    s.name.toLowerCase().includes('treatment')
  );

  const firstHasPrivateSpaces = firstFloorSpaces.some(s =>
...
try {
  ...
  console.log(`   ✅ Spatial strategy: "${mockReasoning.spatialOrganization.strategy}"`);
  console.log(`   ✅ Material recommendations: ${mockReasoning.materialRecommendations.primary}`);
  console.log(`   ✅ Passive strategies: ${mockReasoning.environmentalConsiderations.passiveStrategies.join(', ')}`);
  console.log(`   ✅ Architectural features: ${mockReasoning.architecturalFeatures.length} features`);
...
`;
  ...
  console.log(`   ✅ Reasoning section includes philosophy: ${hasPhilosophy ? 'Yes' : 'No'}`);
  console.log(`   ✅ Reasoning section includes spatial: ${hasSpatial ? 'Yes' : 'No'}`);
  console.log(`   ✅ Reasoning section includes materials: ${hasMaterials ? 'Yes' : 'No'}`);
  console.log(`   ✅ Reasoning section includes environmental: ${hasEnvironmental ? 'Yes' : 'No'}`);
  console.log(`   ✅ Reasoning section includes features: ${hasFeatures ? 'Yes' : 'No'}`);
  console.log(`   ✅ Reasoning section includes structural: ${hasStructural ? 'Yes' : 'No'}`);

  if (hasPhilosophy && hasSpatial && hasMaterials && hasEnvironmental && hasFeatures && hasStructural) {
    console.log('   ✅ PASS: Architectural reasoning fully integrated into DNA prompt');
  } else {
...
try {
  ...
  console.log('   5. ✅ DNA generation → Includes reasoning + wind + constraints');
  ...
  const allPassed = Object.values(workflowChecks).every(check => check === true);
...
console.log('   - Wind speed, direction, and impact extracted from OpenWeather API');
console.log('   - Facade orientation recommendations based on prevailing winds');
...
console.log('   - Auto-correction for violations (scale footprint, reduce height)');
...
console.log('   - Building type-specific rules (healthcare, office, residential)');

console.log('\n✅ Enhancement 4: Architectural Reasoning Integration');
console.log('   - generateDesignReasoning() called before DNA generation');
console.log('   - Reasoning includes philosophy, spatial, materials, environmental');
...
console.log('   - Location → Site → Program → Reasoning → DNA → Validation → A1');
